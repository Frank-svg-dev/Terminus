...
func (m *XFSCLI) CleanProjectID(path string, projectID uint32) error {
	err := exec.Command("xfs_quota", "-x", "-c", fmt.Sprintf("project -C -p %s %d", path, projectID), "/").Run()
	if err != nil {
		return fmt.Errorf("14444", err)
	}
...
func (m *XFSCLI) SetQuota(projectID uint32, limitBytes uint64) error {
	...
	// MVP: 调用 xfs_quota 命令
	cmd := exec.Command("xfs_quota", "-x", "-c", fmt.Sprintf("limit -p bhard=%d %d", limitBytes, projectID), "/")
	if out, err := cmd.CombinedOutput(); err != nil {
...
func (m *XFSCLI) RemoveQuota(dirPath string, projectID uint32) error {
	// mountPoint, err := findMountPoint(dirPath)
	// if err != nil {
	// 	return fmt.Errorf("failed to find mount point: %w", err)
	// }

	// 构造命令：将所有限制设为 0
	// limit -p bsoft=0 bhard=0 isoft=0 ihard=0 <ID>
